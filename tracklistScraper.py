...
trackTable = soup.find_all(class_=constant.TRACK_INFO_DIV_CLASS)
...
linksTable = soup.find_all(class_=constant.MEDIA_LINKS_DIV_CLASS)
# Print binary condition for the track having a spotify link
for link in linksTable:
    spotifyLink = link.find_next(class_=constant.SPOTIFY_LINK_ICON_CLASS)
    trackAvailable.append(constant.MEDIA_LINKS_DIV_CLASS in spotifyLink['class'])
